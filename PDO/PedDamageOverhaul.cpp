...
//______________________________________________________________________________________________________________________________________________________________________________________________________________________________
			...
			//______________________________________________________________________________________________________________________________________________________________________________________________________________________________
				...
				if (peds[i] != playerPed && !isstorychar && PED::IS_PED_HUMAN(peds[i]) && !ENTITY::IS_ENTITY_DEAD(peds[i]))
				{
					...
					float groundzcoordhead, groundzcoordhip, groundzcoordarm1, groundzcoordarm2, groundzcoordleg1, groundzcoordleg2;
			...
			//______________________________________________________________________________________________________________________________________________________________________________________________________________________________
					...
					if ((PED::GET_PED_MAX_HEALTH(peds[i]) == ini_pedhealth || PED::GET_PED_MAX_HEALTH(peds[i]) == ini_pedhealthvehicle) && pedmapstat[peds[i]] != 4 && pedmapstat[peds[i]] != 5)
					{
						...
						else
						{
							...
							{
								if (pedmaphandsup[peds[i]] != 0 && pedmaphandsup[peds[i]] + ini_handsupdelta < GAMEPLAY::GET_GAME_TIMER() && pedmapfirsttimehandsup[peds[i]] == 0)
								{
									if (pedmapfirsttimehandsup[peds[i]] == 0) pedmapfirsttimehandsup[peds[i]] = GAMEPLAY::GET_GAME_TIMER();
								}
								if (pedmapfirsttimehandsup[peds[i]] + ini_handsuptime > GAMEPLAY::GET_GAME_TIMER())
								{
						...
						if (PED::GET_PED_LAST_DAMAGE_BONE(peds[i], &actBone) && pedmaphasbeendamagedbyweapon[peds[i]])
						{
							...
							for (vector<int>::size_type it = 0; it != torsobonesbleeding.size(); it++)
							{
								...
								{
									if (pedmapshotbyplayer[peds[i]] != 0 && GAMEPLAY::GET_GAME_TIMER() < pedmapshotbyplayer[peds[i]] + ini_torsodamagedelay) pedmaptorsohit[peds[i]] = 1;
								}
							}
							...
							if (actBone == 43312 || actBone == 6884 || actBone == 33646)
							{
								...
								pedmaprightleghitcounter[peds[i]]++;
							}
							//left leg
							else if (actBone == 65478 || actBone == 55120 || actBone == 45454)
							{
								...
								pedmapleftleghitcounter[peds[i]]++;
							}
						...
						if (ini_disarmingchance > 0)
						{
							...
							{
								if (pedmaphealth[peds[i]] > (npchealth + 15))
								{
									AI::_0xFD45175A6DFD7CE9(peds[i], playerPed, 4, 0, -1.0f, -1, 0); //fleeing
									pedmaphealth[peds[i]] = 0;
								}
								else if (pedmaphealth[peds[i]] > (npchealth + 5))
								{
							...
							{								
								...
								else if (!isstanding)
								{
									...
									if (pedisready)// && ENTITY::GET_ENTITY_SPEED(peds[i]) < maxragdollspeed)
									{
										...
										{
											if (!AUDIO::IS_ANY_SPEECH_PLAYING(peds[i])) PedPain(peds[i]);
										...
										else
										{
											...
											if (pedmapdyingstate3entered[peds[i]] == 0) //&& pedmapdyingstate1entered[peds[i]] == 0 && pedmapdyingstate2entered[peds[i]] == 0
											{
												...
												AI::CLEAR_PED_TASKS(peds[i], false, false);
												AI::CLEAR_PED_SECONDARY_TASK(peds[i]);
												PED::SET_PED_TO_RAGDOLL(peds[i], 3500, 3500, 3, false, false, false); //ragdoll the NPC
											}
											int painrand = 0 + (std::rand() % (999 - 0 + 1));
											int painaudio = 9;
